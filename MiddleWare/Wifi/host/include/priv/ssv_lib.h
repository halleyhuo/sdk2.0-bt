...
void ssv6xxx_gen_random(u8 *data, u32 len);
bool ssv6xxx_promiscuous_enable(bool enable);//TRUE: pass all data frames to host. FALSE: Keep all data frames in FW.
int ssv6xxx_reduce_phy_cca_bits(void);
int ssv6xxx_recover_phy_cca_bits(void);
#if(ENABLE_DYNAMIC_RX_SENSITIVE==1)
#define MAX_CCI_SENSITIVE 128
void ssv6xxx_update_cci_setting(u16 input_level);
void ssv6xxx_sta_mode_disconnect(void);
void ssv6xxx_sta_mode_connect(void);
#endif
void ssv6xxx_set_TXQ_SRC_limit(u32 qidx,u32 val);
