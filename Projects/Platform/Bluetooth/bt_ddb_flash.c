...
static uint32_t Flash_GetBtDbOffset(void)
{
	...
	SpiFlashRead(DWNLDR_USRDAT_ADDR, (uint8_t*)&start_offset, 4);
...
uint8_t BtDdb_Open(const uint8_t * localBdAddr)
{
	...
	while(1)
	{
		...
		SpiFlashRead(last_offset, tmp_db_flag, sizeof(tmp_db_flag));
		if(memcmp(tmp_db_flag, mvbt_db_flag, sizeof(mvbt_db_flag)) == 0)
		{
			tmp_offset = last_offset;
			tmp_offset += sizeof(mvbt_db_flag);

			SpiFlashRead(tmp_offset, tmp_valid_flag, VALID_OCCUPY_BYTES);
			tmp_offset += VALID_OCCUPY_BYTES;

			SpiFlashRead(tmp_offset, tmp_bdAddr, BDADDR_OCCUPY_BYTES);
			tmp_offset += BDADDR_OCCUPY_BYTES;

			SpiFlashRead(tmp_offset, tmp_count, COUNT_OCCUPY_BYTES);
...
bool BtDdb_GetFirstRecord(BT_DB_RECORD * record)
{
	...
	SpiFlashRead(tempReadAddress, (uint8_t*)record, BTDB_ONE_RECORD_SIZE);
			...
			if(tmp_valid_flag[0] == 1) /*valid db*/
			{
				...
				*/
					...
					*/
					...
					SpiFlashWrite(valid_flag_offset, vaild_flag, VALID_OCCUPY_BYTES);
...
END_FIND:
	...
	*/
	...
	// write the db flag
	SpiFlashWrite(tmp_offset, (uint8_t*)mvbt_db_flag, sizeof(mvbt_db_flag));
	tmp_offset += sizeof(mvbt_db_flag);

	// write the vaild flag
	memset(vaild_flag, 0, VALID_OCCUPY_BYTES);
	vaild_flag[0] = 1;
	SpiFlashWrite(tmp_offset, vaild_flag, VALID_OCCUPY_BYTES);
	tmp_offset += VALID_OCCUPY_BYTES;

	// write local bd address
	SpiFlashWrite(tmp_offset, (uint8_t*)btDdbDbAddr, BDADDR_OCCUPY_BYTES);
	tmp_offset += BDADDR_OCCUPY_BYTES;
	...
	SpiFlashWrite(tmp_offset, count_of_devices, COUNT_OCCUPY_BYTES);
	tmp_offset += COUNT_OCCUPY_BYTES;
...
bool BtDdb_AddOneRecord(const BT_DB_RECORD * record)
{
	...
	SpiFlashWrite(tempWriteAddress, (uint8_t*)record, BTDB_ONE_RECORD_SIZE);
